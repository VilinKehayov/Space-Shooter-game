...
class invaderProjectile {
    constructor({position, velocity}) {
      ...
      //  this.height = 20
        ...
        image3.src = './img/invaderrocket.png'
        image3.onload = () => {
           const scale = 0.5;
           this.image3 = image3
    ...
    draw() {
      c.drawImage(this.image3, this.position.x, this.position.y, this.width, this.height)
     // c.fillStyle = 'white'  
...
class Grid {
    constructor() {
       ...
       const columns = Math.floor(Math.random() * 2 + 2 );
       const rows = Math.floor(Math.random() * 3 + 2 );
       this.width = columns * 210
...
//let randomInterval = Math.floor(Math.random() * 5000 + 200)
let game = {
over: false,
active: true
}
   ...
   switch (key) {
    case 'a':
        ...
        projectiles.push(new Projectile({
            position: {
            x: player.position.x + player.width / 1.4,
            y: player.position.y
            },
            velocity: {
                x: 0,
